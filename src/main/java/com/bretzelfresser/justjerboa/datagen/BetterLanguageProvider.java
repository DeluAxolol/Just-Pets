...
public abstract class BetterLanguageProvider implements DataProvider {
    ...
    public void simpleEntity(EntityType<?> type) {
        this.add(type, toTitleCase(BuiltInRegistries.ENTITY_TYPE.getKey(type).getPath()));
    }

    public void simpleEffect(MobEffect effect) {
        this.add(effect, toTitleCase(BuiltInRegistries.MOB_EFFECT.getKey(effect).getPath()));
    }


    public void simpleMenu(MenuType<?> type) {
        add(type, toTitleCase(BuiltInRegistries.MENU.getKey(type).getPath()));
    }

    public void add(MenuType<?> type, String translation) {
        this.add(Util.makeDescriptionId("menu", BuiltInRegistries.MENU.getKey(type)), translation);
    }

    public void simpleItem(Item item) {
        this.add(item, toTitleCase(BuiltInRegistries.ITEM.getKey(item).getPath()));
    }
    ...
    public void addSimpleBlock(Block key) {
        add(key.getDescriptionId(), toTitleCase(BuiltInRegistries.BLOCK.getKey(key).getPath()));
    }
