package com.bretzelfresser.justjerboa.entity;

import com.bretzelfresser.justjerboa.Config;
...
import com.bretzelfresser.justjerboa.registries.ModTags;
...
public class Jerboa extends ShoulderRidingEntity implements VariantHolder<JerboaVariant> {
    ...
    public @Nullable AgeableMob getBreedOffspring(ServerLevel serverLevel, AgeableMob ageableMob) {
        if (ageableMob instanceof Jerboa otherParent) {
            ...
            JerboaVariant childVariant = WeightedRandom.getRandomItem(serverLevel.getRandom(), serverLevel.registryAccess().registryOrThrow(ModJerboaVariants.JERBOA_VARIANT_REGISTRY_KEY).stream().filter(s -> s.isValidCombination(serverLevel.registryAccess(), this.getVariant(), otherParent.getVariant())).toList()).orElse(this.random.nextDouble() < 0.51 ? this.getVariant() : otherParent.getVariant());
            assert entity != null;
            UUID uuid = this.getOwnerUUID();
            if (uuid != null) {
                entity.setOwnerUUID(uuid);
